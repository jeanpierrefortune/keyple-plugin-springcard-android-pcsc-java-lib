...
object GattAttributesSpringCore : GattAttributes() {
  ...
  val UUID_SPRINGCARD_CCID_SERVICE: UUID
    get() {
      return if (!isCcidServiceBonded) UUID_SPRINGCARD_CCID_PLAIN_SERVICE
      else UUID_SPRINGCARD_CCID_BONDED_SERVICE
    }

  val UUID_CCID_PC_TO_RDR_CHAR: UUID
    get() {
      return if (!isCcidServiceBonded) UUID_CCID_PC_TO_RDR_PLAIN_CHAR
      else UUID_CCID_PC_TO_RDR_BONDED_CHAR
    }

  val UUID_CCID_RDR_TO_PC_CHAR: UUID
    get() {
      return if (!isCcidServiceBonded) UUID_CCID_RDR_TO_PC_PLAIN_CHAR
      else UUID_CCID_RDR_TO_PC_BONDED_CHAR
    }

  val UUID_CCID_STATUS_CHAR: UUID
    get() {
      return if (!isCcidServiceBonded) UUID_CCID_STATUS_PLAIN_CHAR else UUID_CCID_STATUS_BONDED_CHAR
    }
}
