...
internal class BleLowLevel(
  ...
  private var characteristicsToRead: MutableList<BluetoothGattCharacteristic> =
      mutableListOf<BluetoothGattCharacteristic>()
  private var characteristicsCanIndicate: MutableList<BluetoothGattCharacteristic> =
      mutableListOf<BluetoothGattCharacteristic>()
  private var characteristicsToReadPower: MutableList<BluetoothGattCharacteristic> =
      mutableListOf<BluetoothGattCharacteristic>()
      ...
      object : BluetoothGattCallback() {
        ...
        fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
          ...
          if (status != BluetoothGatt.GATT_SUCCESS) {
            scardReaderList.commLayer.onCommunicationError(
                SCardError(
                    ...
                    "Unable to discover GATT, onServicesDiscovered returned: $status"))
            return
          }
          ...
          if (services.isEmpty()) {
            scardReaderList.commLayer.onCommunicationError(
                SCardError(
                    SCardError.ErrorCodes.MISSING_SERVICE,
                    "Android thinks that the GATT of the device is empty"))
            return
          }
          ...
          if (uuidCharacteristicsCanIndicate.size != characteristicsCanIndicate.size ||
            ...
            scardReaderList.commLayer.onCommunicationError(
                SCardError(
                    SCardError.ErrorCodes.MISSING_CHARACTERISTIC,
                    "One or more characteristic are missing in the GATT"))
            return
          }
        ...
        fun onCharacteristicRead(
          ...
          when (characteristic.uuid) {
            GattAttributesSpringCore.UUID_MODEL_NUMBER_STRING_CHAR ->
                scardReaderList.constants.productName =
                    characteristic.value.toString(charset("ASCII"))
            GattAttributesSpringCore.UUID_SERIAL_NUMBER_STRING_CHAR -> {
              scardReaderList.constants.serialNumber =
                  characteristic.value.toString(charset("ASCII"))
              scardReaderList.constants.serialNumberRaw =
                  characteristic.value.toString(charset("ASCII")).hexStringToByteArray()
            }
            GattAttributesSpringCore.UUID_FIRMWARE_REVISION_STRING_CHAR ->
                scardReaderList.constants.softwareVersion =
                    characteristic.value.toString(charset("ASCII"))
            GattAttributesSpringCore.UUID_HARDWARE_REVISION_STRING_CHAR ->
                scardReaderList.constants.hardwareVersion =
                    characteristic.value.toString(charset("ASCII"))
            GattAttributesSpringCore.UUID_SOFTWARE_REVISION_STRING_CHAR -> {
              try {
                ...
                scardReaderList.commLayer.onCommunicationError(
                    SCardError(
                        ...
                        "Incorrect firmware revision: ${characteristic.value.toString(charset("ASCII"))}"))
              }
            }
            GattAttributesSpringCore.UUID_MANUFACTURER_NAME_STRING_CHAR ->
                scardReaderList.constants.vendorName =
                    characteristic.value.toString(charset("ASCII"))
            GattAttributesSpringCore.UUID_PNP_ID_CHAR ->
                scardReaderList.constants.pnpId = characteristic.value.toHexString()
            GattAttributesSpringCore.UUID_CCID_STATUS_CHAR -> {
              ...
              if (slotCount.toInt() == 0) {
                scardReaderList.commLayer.onCommunicationError(
                    SCardError(SCardError.ErrorCodes.DUMMY_DEVICE, "This device has 0 slots"))
                return
              }
              ...
              error =
                  scardReaderList.ccidHandler.interpretCcidStatus(
                      readCcidStatus, listSlotsUpdated, isNotification = false)
        ...
        override fun onCharacteristicWrite(
          ...
          if (status != BluetoothGatt.GATT_SUCCESS) {
            scardReaderList.commLayer.onCommunicationError(
                SCardError(
                    SCardError.ErrorCodes.WRITE_CHARACTERISTIC_FAILED,
                    "Failed to write on characteristic $characteristic"))
            return
          }
        ...
        override fun onDescriptorWrite(
          ...
          if (status != BluetoothGatt.GATT_SUCCESS) {
            scardReaderList.commLayer.onCommunicationError(
                SCardError(
                    SCardError.ErrorCodes.ENABLE_CHARACTERISTIC_EVENTS_FAILED,
                    "Failed to subscribe to notification for characteristic ${descriptor.characteristic}"))
            return
          }
    ...
    return if (txBufferCursorBegin < txBuffer.size) {
      ...
      if (!mBluetoothGatt.writeCharacteristic(charCcidPcToRdr)) {
        scardReaderList.commLayer.onCommunicationError(
            SCardError(
                SCardError.ErrorCodes.WRITE_CHARACTERISTIC_FAILED,
                "Failed to write in characteristic ${charCcidPcToRdr.uuid}"))
        return true
      }
  ...
  fun interpretFirstByteCcidStatusBle(data: Byte): SCardError {
    ...
    val isSleeping =
        data and CcidHandler.LOW_POWER_NOTIFICATION == CcidHandler.LOW_POWER_NOTIFICATION
          ...
          if (BluetoothAdapter.ACTION_STATE_CHANGED == action) {
            when (intent.getIntExtra(BluetoothAdapter.EXTRA_STATE, -1)) {
              BluetoothAdapter.STATE_OFF -> {
                scardReaderList.commLayer.onCommunicationError(
                    SCardError(
                        SCardError.ErrorCodes.DEVICE_NOT_CONNECTED, "Bluetooth has been disabled"))
