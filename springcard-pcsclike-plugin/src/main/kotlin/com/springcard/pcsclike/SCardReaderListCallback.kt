...
abstract class SCardReaderListCallback {
  ...
  /**
   * When a disconnection from the current connected readerList is asked or when the readerList
   * itself close
  ...
  /**
   * Invoked for all readerList-level errors, e.g. BLE error, protocol error, etc. When this
   * callback is invoked, the connection to the readerList is often closed.
   * @param readerList SCardReaderList (could be null if the ScardReaderList has not been created
   * yet)
  ...
  /**
   * Invoked for all “recoverable” errors, e.g. invalid slot number, card absent, card removed or
   * mute, etc.
