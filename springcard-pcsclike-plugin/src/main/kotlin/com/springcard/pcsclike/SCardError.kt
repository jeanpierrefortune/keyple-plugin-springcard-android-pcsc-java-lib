...
/**
 * Class used by the errors callbacks when an error occurred It contains some information about how
 * the error happened
 *
...
class SCardError(val code: ErrorCodes, val detail: String = "", val isFatal: Boolean = false) {
  ...
  enum class ErrorCodes(val value: Int) {
    ...
    /**
     * The library has been called to instantiate a device (SCardReaderList object), but the
     * provided input parameter does not refer to a connected BLE device. Or the underlying BLE
    ...
    /**
     * One of the mandatory services has not the expected settings (for instance, bonding flags not
     * consistent with the implementation in the library)
     */
    INVALID_SERVICE_SETTINGS(0x1500),
    /**
     * One of the mandatory characteristics has not the expected settings (for instance,
     * read/write/notify flags not consistent with the implementation in the library)
     */
    ...
    /**
     * The underlying BLE device is still connected, but a CCID-level timeout has occurred (no
     * RDR_To_PC after a PC_To_RDR)
     */
    COMMUNICATION_TIMEOUT(0x1A00),
    /**
     * The format of the RDR_To_PC or CCID_Status characteristic is invalid (unsupported opcode, bad
     * length etc)
     */
    PROTOCOL_ERROR(0x1B00),
    /**
     * The device does not answer as expected (ex: empty response to a SCardControl that is looking
     * for the slot name)
     */
    DIALOG_ERROR(0x1C00),
    ...
    CARD_MUTE(0x3300),
    /**
     * Transmit or Disconnect invoked, but the library is aware that the card has already been
     * removed
     */
    CARD_REMOVED(0x3400),
    /**
     * Transmit invoked, and no response from the card (the card is likely to have been removed
     * during the exchange)
     */
