...
internal class CcidHandler(private val scardReaderList: SCardReaderList) {
  ...
  fun updateCcidCommand(command: CcidCommand): ByteArray {
    ...
    commandSend =
        when (tmp.code) {
          CcidCommand.CommandCode.PC_To_RDR_IccPowerOn.value ->
              CcidCommand.CommandCode.PC_To_RDR_IccPowerOn
          CcidCommand.CommandCode.PC_To_RDR_IccPowerOff.value ->
              CcidCommand.CommandCode.PC_To_RDR_IccPowerOff
          CcidCommand.CommandCode.PC_To_RDR_GetSlotStatus.value ->
          ...
          CcidCommand.CommandCode.PC_To_RDR_XfrBlock.value ->
              CcidCommand.CommandCode.PC_To_RDR_XfrBlock
          else ->
              throw Exception(
                  "Impossible value for CommandCode : 0x${String.format("%02X", tmp.code)})")
        }
  ...
  fun interpretCcidStatus(
      data: ByteArray,
      updatedReaders: MutableList<SCardReader>,
    ...
    if (data.isEmpty()) {
      return SCardError(
          SCardError.ErrorCodes.PROTOCOL_ERROR, "Error, interpretSlotsStatus: array is empty")
    }
    ...
    for (i in 0 until data.size) {
      for (j in 0..3) {
        ...
        if (slotNumber < scardReaderList.readers.size) {
          ...
          slot.cardPresent =
              !(slotStatus == SCardReader.SlotStatus.Absent.code ||
          ...
          val cardRemoved =
              (slotStatus == SCardReader.SlotStatus.Removed.code) ||
                  (!isNotification && !slot.cardPresent)
          /* Card considered inserted if we read the CCID status and card is present */
          val cardInserted =
              (slotStatus == SCardReader.SlotStatus.Inserted.code) ||
                  (!isNotification && slot.cardPresent)
   ...
   */
  fun interpretSlotsErrorInCcidHeader(
      slotError: Byte,
      slotStatus: Byte,
      slot: SCardReader
  ): SCardError {
